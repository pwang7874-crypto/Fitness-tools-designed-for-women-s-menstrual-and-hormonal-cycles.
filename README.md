...
> **在线体验：** [打开顺期健身 App](https://sbcejds882ht7of2dtslu.apigateway-cn-beijing.volceapi.com/)
>
> **API 地址：** <https://snc79cdfrhj86otkgqnhe.apigateway-cn-beijing.volceapi.com/>
...
- 数据备份：TOS 私有桶，每 300 秒生成一次 SQLite 一致性快照，新实例启动时自动恢复

线上环境启用了管理员发放的邀请码、Secure/HttpOnly 设备私有会话、弹性实例和 TOS 数据库备份。弹性实例缩容后首次访问可能出现冷启动。当前仍是受邀内测版本，周期推算和训练建议仅供日历与健身参考，不构成医疗诊断、排卵确认或避孕依据。
...
当前是已上线的受邀内测 Web MVP，不等于生产完成。当前 SQLite + TOS 定时快照适合小规模内测；正式开放前仍建议迁移到托管数据库，并补齐正式账号恢复/多设备登录、CSRF 和速率限制、错误监控、运动科学评审及浏览器端旅程测试。详见 docs/技术适配声明_V0.2.md。
